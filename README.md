...
> [!NOTE]
> **occam-bci fork.** Periphery 3.8.0 plus XcodeProj 9.16.0, so `periphery scan` can read projects generated by Tuist ≥ 4.206.0 (`com.apple.product-type.tool.host-build`). Releases are SwiftPM-built arm64 binaries under [Releases](https://github.com/occam-bci/periphery/releases); with mise: `"github:occam-bci/periphery" = "3.8.1"`. The Bazel build resolves XcodeProj through a root-module `archive_override` because the Bazel Central Registry stops at 9.10.1; a Bazel project that consumes this fork as a module must add the same override, with `bazel/xcodeproj/add_module_and_build_files.patch`, to its own `MODULE.bazel`, since Bzlmod honours overrides only in the root module.
